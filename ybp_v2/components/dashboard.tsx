...
//     () => import('echarts-for-react') as any, // or as unknown
//     { ssr: false }
// );
// import dynamic from 'next/dynamic';
import ReactECharts from 'echarts-for-react'; // 或 import {ECharts} from 'echarts-for-react'; 取决于库的导出方式

// const ChartComponent = dynamic(() => import('echarts-for-react'), { ssr: false });
...
// import type { EChartsReactProps } from 'echarts';
import type { EChartsReactProps } from 'echarts-for-react';
export function DashboardComponent() {
  const [isExpanded, setIsExpanded] = useState(false)
  const [currentMonth, setCurrentMonth] = useState(0)
  const [monthlyData] = useState(Array(24).fill(null).map(() => generateRandomData()))
  ...
  const barChartRef = useRef<EChartsReactProps>(null)
  const pieChartRef = useRef<EChartsReactProps>(null)
  const liquidChartRefs = [useRef(null), useRef(null), useRef(null), useRef(null)]

  useEffect(() => {
    let dataIndex = 0;
    const tooltipInterval = setInterval(() => {
      const barChart = barChartRef.current?.echartsInstance;
      const pieChart = pieChartRef.current?.echartsInstance;
