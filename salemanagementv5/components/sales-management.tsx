...
interface MyEchartOption extends echarts.EChartOption {
  useUTC?: boolean; // 添加 useUTC 属性
}
const menuItems = [
...
const salesBrands = ["苹果", "三星", "华为", "小米", "OPPO"]
...
export function SalesManagementComponent() {
  ...
  useEffect(() => {
    ...
    const myEchartOption : EChartOption {
      title: { text: '销售趋势分析' },
      tooltip: {
        trigger: 'axis',
    ...
    if (selectedMonth !== "all" && selectedBrand === "all") {
      ...
      const salesTrendOption = echarts.EChartOption(myEchartOption)
    ...
    const intervalId = setInterval(() => {
      currentIndex = (currentIndex + 1) % (selectedMonth !== "all" && selectedBrand === "all" ? salesBrands.length : months.length)
        ...
        <main className="p-6">
          <Tabs defaultValue="orders" className="w-full">
            <TabsList>
              ...
              <TabsTrigger value="analysis">销售统计分析</TabsTrigger>
            </TabsList>
